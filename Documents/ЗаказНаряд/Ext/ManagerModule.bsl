&НаСервере
Процедура ПечатьЗаказНаряда(ТабДок, Ссылка) Экспорт
	
	Макет = Документы.ЗаказНаряд.ПолучитьМакет("ПечатьЗаказНаряда");
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьПодвал.Параметры.СуммаПрописью = ЧислоПрописью(Выборка.СуммаДокумента, "Л=ru_RU", ПараметрыВалюты);
